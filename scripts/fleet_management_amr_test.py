...
class FleetManagerAMR:
    def __init__(self):
        ...
        self.publishers = {bot_name: rospy.Publisher(f"/{bot_name}/target_zone", String, queue_size=10) for bot_name in self.target_zones.keys()}
    ...
    def update_target_zones(self, part_spawner):
        for robot_namespace, zones in self.target_zones.items():
            ...
            has_model = part_spawner.check_model(target_zone_value['part'], robot_namespace)
...
class PartSpawner:
    def __init__(self):
        ...
        self.zones = self.load_zone_coordinates(self.rospack.get_path('bcr_bot') + "/param/zone_coordinates.yaml")
    ...
    def main(self, fleet_manager_amr):
        for robot_namespace in self.robots:
            target_zone = fleet_manager_amr.target_zones[robot_namespace][fleet_manager_amr.current_zones[robot_namespace]]
            target_zone_value = self.zones[target_zone]
            is_at_zone = self.check_robot_reached_zone(robot_namespace, target_zone_value)
            is_supply_zone = target_zone.startswith('S')
            is_demand_zone = target_zone.startswith('D')
            has_model = self.check_model(target_zone_value['part'], robot_namespace)

            if not has_model and is_at_zone and is_supply_zone:
                rospy.loginfo(f"{robot_namespace} 로봇이 {target_zone} 구역에서 부품 {target_zone_value['part']}를 수령했습니다.")
                spawn_point = Point(x=target_zone_value['x'], y=target_zone_value['y'], z=0.5)
                self.spawn_model_func(target_zone_value['part'], robot_namespace, spawn_point, [0, 0, 0])

            elif has_model and is_at_zone and is_demand_zone:
                rospy.loginfo(f"{robot_namespace} 로봇이 {target_zone} 구역에서 부품 {target_zone_value['part']}를 공급했습니다.")
    ...
    def spawn_model_func(self, part, robot_namespace, spawn_point, spawn_angle):
        with open(self.path + part + "/model.sdf", "r") as f:
            part_sdf = f.read()
        quat = tf.transformations.quaternion_from_euler(spawn_angle[0], spawn_angle[1], spawn_angle[2])
    ...
    def get_pose(self, robot_namespace):
        ...
        try:
            listener.waitForTransform('map', f'{robot_namespace}/base_footprint', rospy.Time(), rospy.Duration(10.0))
            (trans, rot) = listener.lookupTransform('map', f'{robot_namespace}/base_footprint', rospy.Time(0))
